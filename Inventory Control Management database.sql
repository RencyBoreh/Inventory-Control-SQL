create table Categories(
 ...
 Caterogy_Name VARCHAR (55)
 );
 alter table Categories
 add Category_Name VARCHAR(55);
 create table Product(
 Product_id INT PRIMARY KEY,
 Product_Name varchar(55),
 Category_id INT,
 Unit_price DECIMAL(10,2),Units_instock INT ,
...
-- updating product:queries to update the product information
-- a.based on the unit price
UPDATE Product 
...
INSERT INTO Sales(SaleID, Product_id, SalesDate, QuantitySold) VALUES
...
SET Product.Units_instock = 
	...
	END; 
    SELECT * FROM Product;
    -- Data Integrity: Implementing foreign key constraints
    ALTER TABLE Product 
    ...
    FOREIGN KEY (Product_id) REFERENCES Product(Product_id);
    -- Data Validation: Ensuring positive unit price and non-negative quantity sold
    ALTER TABLE Product ADD CONSTRAINT chk_unit_price CHECK (Unit_price > 0);
    ALTER TABLE Sales ADD CONSTRAINT chk_quantity_sold CHECK (QuantitySold >= 0);
    -- Reports: Generating total sales by product
    SELECT p.Product_Name, SUM(s.QuantitySold) AS Total_Quantity_Sold, SUM(s.QuantitySold * p.Unit_price) AS Total_Sales FROM Product p JOIN Sales s ON p.Product_id = s.Product_id
    GROUP BY p.Product_Name ORDER BY Total_Sales DESC;
    -- Current Inventory Status
    SELECT Product_Name, Units_instock FROM Product ORDER BY Units_instock ASC;
    -- Query to check the integrity of data relationships
    SELECT * FROM Product WHERE Category_id NOT IN (SELECT Category_id FROM Categories);
    -- Sample queries for basic operations and reports 
    -- Basic Operation: Retrieve all products in the 'Electronics' category
    SELECT p.Product_Name, p.Unit_price, p.Units_instock FROM Product p JOIN Categories c ON p.Category_id = c.Category_id
